...
static int sanitize_kit_runtime_flags(Part::Kit *kits)
{
    int fixed_count = 0;
    for(int i = 0; i < NUM_KIT_ITEMS; ++i) {
        auto &k = kits[i];
        if(k.Padenabled && !k.adpars) {
            k.Padenabled = false;
            ++fixed_count;
#ifdef __ANDROID__
            __android_log_print(ANDROID_LOG_WARN, "zynbridge",
                                "sanitize kit[%d]: disabled AD synth flag (missing adpars)",
                                i);
#else
            fprintf(stderr,
                    "ZynAddSubFX: sanitize kit[%d]: disabled AD synth flag (missing adpars)\n",
                    i);
#endif
        }
        if(k.Ppadenabled && !k.padpars) {
            k.Ppadenabled = false;
            ++fixed_count;
#ifdef __ANDROID__
            __android_log_print(ANDROID_LOG_WARN, "zynbridge",
                                "sanitize kit[%d]: disabled PAD synth flag (missing padpars)",
                                i);
#else
            fprintf(stderr,
                    "ZynAddSubFX: sanitize kit[%d]: disabled PAD synth flag (missing padpars)\n",
                    i);
#endif
        }
        if(k.Psubenabled && !k.subpars) {
            k.Psubenabled = false;
            ++fixed_count;
#ifdef __ANDROID__
            __android_log_print(ANDROID_LOG_WARN, "zynbridge",
                                "sanitize kit[%d]: disabled SUB synth flag (missing subpars)",
                                i);
#else
            fprintf(stderr,
                    "ZynAddSubFX: sanitize kit[%d]: disabled SUB synth flag (missing subpars)\n",
                    i);
#endif
        }
    }
                  ...
                  float note_log2_freq)
{
    ...
    const int sanitized = sanitize_kit_runtime_flags(kit);
    if(sanitized > 0) {
        // Standalone Android bring-up can encounter malformed kit state after
        // aggressive preset switching. Prefer dropping invalid kit flags over
        // aborting the audio thread.
#ifdef __ANDROID__
        __android_log_print(ANDROID_LOG_WARN, "zynbridge",
                            "sanitized %d invalid kit flag(s) before NoteOn", sanitized);
#else
        fprintf(stderr, "ZynAddSubFX: sanitized %d invalid kit flag(s) before NoteOn\n", sanitized);
#endif
    }
