    ...
    synth(synth_), gzip_compression(config->cfg.GzipCompression)
{
    ...
    midi.frontend = [this](const char *msg) { if(bToU) bToU->raw_write(msg); };
                           ...
                           Master* master_from_mw)
{
    ...
    if(!d.matches && !d.forwarded) {// && !ports.apropos(msg)) {
        ...
        const char *peak = uToB ? uToB->peak() : "<no-uToB>";
        fprintf(stderr, "Unknown address<BACKEND:%s> '%s:%s'",
                offline ? "offline" : "online",
                peak,
                uToB ? rtosc_argument_string(peak) : "");
        fprintf(stderr, "%c[0m\n", 0x1B);
        if(unknown_address_cb)
            unknown_address_cb(unknown_address_cb_ptr, offline, uToB ? uToB->peak() : nullptr);
    }
    else if(d.forwarded)
        if(bToU)
            bToU->raw_write(msg);
